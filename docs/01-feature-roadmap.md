...
>
> **Status (2026-07-05):** checkboxes below reflect actual shipped state (this doc had never been updated as work landed — the `[ ]`s were stale, not accurate; a second pass the same day also found and corrected a few `[x]`s that had been marked done *before* the code actually existed — see the code-snippet and export-workspace lines below for what that looked like). A full "Auth & protocols" batch shipped this same day: OAuth 1.0, custom proxy, a cookie-jar UI, batch send (scoped to run-folder), 1Password integration (via the `op` CLI), GraphQL schema introspection, and gRPC server-streaming. Roughly: **v0.1 MVP is done**, **v0.5 is essentially done** (only NTLM auth and auto-update mechanics remain — NTLM deliberately deferred, see its own line below: it's a challenge-response handshake that doesn't fit this app's "compute once, attach a header" auth model the way every other method here does, and is a narrow use case for a macOS-only app), **v1.0 is partial** (gRPC has unary + server-streaming, no client-streaming/bidi/`.proto` import; GraphQL has schema introspection, no autocomplete-while-typing; MCP Resources and full capability-grant UI are missing; no plugin runtime), **v2.0 is not started** (no Windows/Linux, no mock server, no HTTP/3, no cloud sync). See `api-tool-build-progress` session memory for the full shipped-feature list with dates.
...
- [x] OAuth 2.0 — client-credentials (2026-07-05, token caching added 2026-08-31) AND authorization-code + PKCE through the system browser (2026-08-31, v0.6.0): RFC 8252 loopback flow, keychain-stored tokens, silent refresh with rotation support, per-config single-flighting. Adversarially reviewed (48 verification agents); five confirmed findings fixed with regression tests, including redaction of every auth-minted credential from the script sandbox.
- [x] JWT auth
...
- [ ] NTLM — deliberately deferred (2026-07-05), not attempted: every auth method here (Basic/Bearer/APIKey/JWT/OAuth2/SigV4/OAuth1) fits `auth.Apply`'s "compute once, attach a header, send" model. NTLM is a challenge-response HANDSHAKE — the first request gets a 401 + a `WWW-Authenticate: NTLM <challenge>`, the client computes a response FROM that specific challenge, and resends — so it can't be a new `case` in the existing switch; it needs the HTTP execute loop itself (`internal/protocols/http/http.go`) to detect the 401 and transparently retry, a materially more invasive change than every other auth method combined. Layered on that: NTLM is used almost exclusively for on-prem Windows/Active Directory/Exchange endpoints — a real but narrow use case for a macOS-only app today. Recommending this gets its own focused pass later rather than a same-day attempt.
...
- [x] 1Password integration (2026-07-05) — `internal/onepassword` shells out to the user's own `op` CLI (never bundled/vendored, unlike k6 — a per-user authenticated tool). An environment variable's value can be an `op://vault/item/field` reference instead of a literal; resolved lazily (only for a variable a request actually references, so one broken reference elsewhere in the same environment can't fail unrelated requests) inside `internal/templating`, so every existing `${varName}` use site (headers, URL, body, params) gets 1Password support with no auth-method-specific wiring. The environment editor shows a live "1Password reference" / "⚠ op not found" hint next to any `op://` value. Scope: variables only, not a literal `op://` value pasted directly into an auth method's own field (auth fields aren't run through the templater at all today — a separate, pre-existing gap). `op` is confirmed not installed in the dev environment this was verified in, so the real, honest test is the graceful-absence path: a clear "op not found, install it" error naming the specific variable and reference, verified both as a Go test and live through the GUI (including confirming an unrelated request in the same environment is unaffected).
...
| # | Sub-task | Notes |
|---|----------|-------|
| 1.1 | `download-k6.sh` build/Taskfile step → pinned k6 per target triple into the `.app` bundle `Contents/Resources/` (NOT `go:embed`-ed into the Go binary), checksum-verified | **Done 2026-08-29.** `scripts/bundle-k6.sh` stages the pinned k6 **v0.54.0** at `Contents/Resources/bin/k6`, arch auto-detected from the app binary via `lipo -archs` (arm64-only today, matching the app), SHA-256-verified against the digest in `build/sidecars/k6-NOTICE.txt`, with `k6-LICENSE.txt` + `k6-NOTICE.txt` staged alongside. Hand-rolled per-arch staging, as predicted — no Wails equivalent to Tauri `externalBin`. Called from `scripts/release.sh` after `wails build`, before signing. Self-heal fallback for a build that missed it: `DownloadK6` (`app_k6.go` → `internal/perf/download.go`) fetches the same pinned release, verifies the archive **and** the extracted binary against pinned digests, and installs to `~/Library/Application Support/AUK/bin/k6`, which `ResolveK6` now checks (below the repo sidecar, above PATH) |
| 1.2 | Stage + individually codesign the k6 sidecar binary (bottom-up), then notarize the whole bundle (macOS Gatekeeper) | **Scripted 2026-08-29, not yet exercised on a real release.** `scripts/release.sh` signs `Resources/bin/k6` on its own first (`--force --options runtime --timestamp`), then the bundle with `build/darwin/entitlements.plist` — deliberately **not** `--deep` (deprecated; applies the outer entitlements to nested binaries). Correction to the original note: sobek is a pure-Go *interpreter*, not a JIT, so k6 needs no entitlements of its own; the parent's `disable-library-validation` is what allows exec'ing it. Entitlements are indeed not inherited |
...
| 4.4 | **[x] Shipped 2026-08-31 (v2).** Read/discovery tools: `list_workspaces`, `list_environments`, `resolve_variables`, `list_requests`, `get_request`, `search_requests`, `list_folders`, `get_last_response` | Secrets are NEVER returned: `resolve_variables` redacts keychain values to `[secret:name]` (name kept, so an agent explains rather than invents) and `get_request` returns the auth SCHEME, never the credential. `get_last_response` exists so an agent diagnosing a failure does not RE-FIRE a POST just to look at it. |
| 4.5 | **[~] Partly shipped 2026-08-31.** `run_request`, `run_folder`, `run_perf_test` all land. Still open: streaming progress notifications, websocket/gRPC tools, `cancel_run` | `run_folder` is the same path `auk run-folder` takes in CI, so "does the smoke suite pass?" gets CI's answer, not an approximation |
| 4.6 | **[x] Shipped 2026-08-31 (v2).** `create_request`, `update_request`, `delete_request`, `create_folder`, `set_environment_variable` | Every call passes a `WriteGuard`, deliberately separate from the request PolicyEngine — one asks "may this agent send a DELETE?", the other "may it rewrite your collection?". `update_request` uses pointer fields so an omitted field means UNCHANGED (otherwise changing a URL silently wiped headers and body). `set_environment_variable` refuses SECRET names outright — the value would land in git-tracked YAML or move into the keychain invisibly. |
...
| 4.8 | **[x] Shipped 2026-08-31 (v2).** Scopes `read-only` (8 tools) / `run` (11, default) / `write` (16), via `-scope` or `APITOOL_MCP_SCOPE`; GUI defaults to `write` since it can ask per edit | Scope decides which tools are REGISTERED, not which calls are refused: a read-only server's `tools/list` simply lacks `create_request`, so an agent plans against what it can do rather than discovering the boundary by being told no — and a refusal is never mistaken for a transient error worth retrying. Not yet narrowable per workspace/env. |
...
| Auth | Basic, Bearer, API Key, OAuth 2.0 (both grants), JWT, SigV4 + AWS profiles/SSO |
...
| Auth | Add OAuth 1.0, AWS SigV4, NTLM, client certs, 1Password — **all shipped except NTLM** (deliberately deferred, see its own line above) |
